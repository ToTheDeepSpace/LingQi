...
test('keeps the review guide aligned with the enforced server-only flow', () => {
  ...
  const packageJson = JSON.parse(readFileSync('miniapp/jumulu/package.json', 'utf8')) as { version: string };
  ...
  assert.match(guide, new RegExp(versionPattern));
  ...
  assert.match(readme, new RegExp(`当前 ${versionPattern} 范围`));
