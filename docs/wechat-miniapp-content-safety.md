...
1. 确认微信后台开发版为 `0.1.40`，上传回执包体为 614853 字节；红黑榜信息流卡片和详情页均可直接进入举报流程，退出状态不会显示可填写的举报表单。
...
- 本文对应微信开发版 `0.1.40`（614853 字节）；完整测试 225/225、微信小程序与跨端安全专项 67/67 通过。实时生产提交号以共享状态和部署记录为准，避免在同一提交内维护自指版本号。
