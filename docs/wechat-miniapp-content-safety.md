...
- `public-ugc`：会公开展示的用户内容，服务端必须强制调用微信文字检查；公开图片还要经过异步图片检查和业务提交复核。
- `private-content`：只在当事人和管理员之间流转的申诉、申请、举报或联系方式，不公开展示；按最小必要原则处理，不把私密证据发送给微信。
- `state-only`：关注、已读、上下架、撤回、接受或拒绝等不包含新公开言论的状态变更。
- `safety-infrastructure`：微信内容安全预检本身。

`tests/miniappMutationClassification.test.ts` 会扫描小程序源码中的全部非 GET 请求。新增写接口却未分类，或者公开 UGC 没有服务端微信检查时，测试会直接失败。
