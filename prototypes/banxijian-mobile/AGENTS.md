...
本轮实现消息中心与层级返回；全局消息不能直接打开个人咨询。以 Apple HIG 为交互验收基准，同时适配微信宿主与 Android，根栏目不加虚假返回，主要触控目标至少 44px。保留“散场后，那段没说完的事”。百科与红黑榜重点是店家和 DM 的评分及口碑，剧本与角色保留为关联资料，不删除历史数据；社区评分不能和保证金、付费认证或黑榜事件混算。前100位审核通过的委托师每人1个委托条免费365天，从首次审核通过起算，修改／重上不延长，到期不自动扣费。用户已同意先发布评分、消息导航和免费上架，再接交易闭环；本目录所有交易仍为虚构演示，不得当真实交易发布。
