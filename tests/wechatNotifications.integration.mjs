...
const migration = readFileSync(new URL('../supabase/migrations/20260905210000_wechat_notification_delivery.sql', import.meta.url), 'utf8');
...
async function consent(db, who = owner) {
  const call = routesFor(db);
  const request = await call('post','wechat-notifications/requests',{ owner: who });
...
async function notice(db) {
  return (await db.query("insert into lc_account_notifications(profile_id,type,title,content) values($1,'commission_received','private title','private evidence') returning id",[owner])).rows[0].id;
}
