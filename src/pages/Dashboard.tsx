                ...
                <MineSection title="红黑白榜" emptyText="还没有发布过口碑">
                  {myRankings.map(item => (
                    <MineRow key={item.id}
                      ...
                      meta={`${item.type === 'red' ? '红榜' : item.type === 'black' ? '黑榜' : '白榜'} · ${item.subject_city || '未填城市'} · ${item.initial_amount === 0 ? '免费' : `${item.initial_amount} 契约币`} · ${item.type === 'black' ? `赞同${item.likes || 0} 反对${item.dislikes || 0}` : `赞扬${item.likes || 0} 欢乐${item.joys || 0}`}`}
                      status={item.status}
                      to="/rankings"
