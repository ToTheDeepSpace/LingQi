...
app.get('/api/lc/dm-dossiers', async (req, res) => {
  try {
    ...
    if (dossierIds.length > 0) {
      const [dmRatingResult, storeRatingResult] = await Promise.all([
