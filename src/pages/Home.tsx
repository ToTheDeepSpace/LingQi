  ...
  return (
    <div style={{ backgroundColor: C, color: '#fff' }}>
      ...
      <section style={{ minHeight: '100svh', backgroundColor: C, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', padding: '6rem 1.25rem 4rem' }}>
        ...
        {[680, 500, 340, 200].map((s, i) => (
          <div key={s} style={{ position: 'absolute', width: s, height: s, borderRadius: '50%', border: `1px solid rgba(201,146,46,${0.06 + i * 0.03})`, pointerEvents: 'none', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} className="hidden sm:block" />
        ))}
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 700, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(201,146,46,0.22)', color: 'rgba(217,168,87,0.75)', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 36 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: GOLD, animation: 'pulse 2s infinite' }} />
            灵契师与委托人的连接之所
          </div>
          ...
          <p style={{ fontSize: 'clamp(0.95rem, 2.5vw, 1.15rem)', color: 'rgba(226,238,252,0.78)', lineHeight: 1.8, marginBottom: 12, textAlign: 'center', maxWidth: 560 }}>
            灵契师，是与虚拟人签订契约的人：让角色借由自己附身片刻，陪委托人走过一段真实时间。
          </p>
          <p style={{ fontSize: 'clamp(0.85rem, 2vw, 1rem)', color: 'rgba(226,238,252,0.66)', lineHeight: 1.8, marginBottom: 36, textAlign: 'center', maxWidth: 560 }}>
          ...
          <p style={{ marginTop: 24, fontSize: 11, letterSpacing: '0.08em', color: 'rgba(226,238,252,0.5)', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            {classes.map(({ icon, role, sub, desc, accent }) => (
              <div key={role} style={{ backgroundColor: `${accent}0d`, border: `1px solid ${accent}33`, borderTop: `3px solid ${accent}`, borderRadius: 16, padding: '28px 20px', textAlign: 'center', transition: 'transform 0.2s, box-shadow 0.2s' }}
                ...
                <p style={{ fontSize: '0.8rem', color: 'rgba(226,238,252,0.68)', lineHeight: 1.7 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ backgroundColor: C2, padding: '5rem 1.25rem' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(201,146,46,0.7)', marginBottom: 12 }}>为什么选择灵契</p>
          <h2 style={{ fontFamily: 'var(--font-serif)', fontWeight: 900, fontSize: 'clamp(1.6rem, 4vw, 2.4rem)', marginBottom: 16 }}>不只是个人主页</h2>
          <div className="gold-line" style={{ margin: '0 auto 48px' }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 32 }}>
            {features.map(({ icon, title, desc }) => (
              <div key={title} style={{ textAlign: 'center' }}>
                ...
                <h3 style={{ fontWeight: 700, fontSize: '1rem', color: '#fff', marginBottom: 10 }}>{title}</h3>
                <p style={{ fontSize: '0.82rem', color: 'rgba(226,238,252,0.68)', lineHeight: 1.8 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 32 }}>
            {steps.map(({ n, title, desc }) => (
              <div key={n} style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.8rem', color: 'rgba(226,238,252,0.64)', lineHeight: 1.7 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '5rem 1.25rem', background: `linear-gradient(135deg, #132b4a 0%, ${C} 50%, #132b4a 100%)`, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'rgba(226,238,252,0.68)', fontSize: '1.05rem', marginBottom: 36, lineHeight: 1.8 }}>无论你是委托人，还是灵契师——这里都为这场契约留好了入口。</p>
      ...
      <footer style={{ backgroundColor: C, borderTop: '1px solid rgba(201,146,46,0.1)', padding: '2.5rem 1.25rem' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, textAlign: 'center', color: 'rgba(226,238,252,0.58)', fontSize: '0.875rem' }}>
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            <Link to="/explore" style={{ color: 'rgba(226,238,252,0.62)', textDecoration: 'none' }} onMouseEnter={e => (e.currentTarget.style.color = '#fff')} onMouseLeave={e => (e.currentTarget.style.color = 'rgba(226,238,252,0.62)')}>进入大厅</Link>
            <Link to="/login" style={{ color: 'rgba(226,238,252,0.62)', textDecoration: 'none' }} onMouseEnter={e => (e.currentTarget.style.color = '#fff')} onMouseLeave={e => (e.currentTarget.style.color = 'rgba(226,238,252,0.62)')}>入驻</Link>
          </div>
        </div>
      </footer>
    </div>
