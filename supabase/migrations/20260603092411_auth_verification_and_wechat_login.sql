-- Phone verification and WeChat login support.
...
create unique index if not exists lc_profiles_wechat_openid_unique
  on public.lc_profiles (wechat_openid)
  where wechat_openid is not null;

create unique index if not exists lc_profiles_wechat_unionid_unique
  on public.lc_profiles (wechat_unionid)
  where wechat_unionid is not null;

create index if not exists lc_profiles_phone_verified_idx
  on public.lc_profiles (phone_verified_at desc)
