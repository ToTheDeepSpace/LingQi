...
10. 微信官方限制单张媒体不超过 10MB，且用于检查的 OpenID 需要在近两小时访问过小程序。令牌失效返回 `40001`、`40014` 或 `42001` 时，服务端只清除对应旧缓存并重试一次；其他错误保持失败关闭。
...
- 本文对应微信开发版 `0.1.37`；完整测试 192/192、微信小程序与跨端安全专项 54/54 通过。实时生产提交号以共享状态和部署记录为准，避免在同一提交内维护自指版本号。
